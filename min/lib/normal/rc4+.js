...
function body(inp, gksa, container, length) {
    for (var a, b, c, i = 0, j = 0, out = container, ksa = gksa.slice(), y = 0; y < length; ++y) b = ksa[j = ksa[(j + (a = ksa[i = (i + 1) % 256])) % 256]], 
    ksa[j] = [ a, ksa[i] = b ][0], c = (ksa[i << 5 ^ j >> 3] + ksa[j << 5 ^ i >> 3]) % 256, 
...
Rc4p.prototype.change = function(key) {
    ...
    for (var a, b, c, i = 0, j = 0, out = "", ksa = this.ksa.slice(), y = 0, l = str.length; y < l; ++y) b = ksa[j = ksa[(j + (a = ksa[i = (i + 1) % 256])) % 256]], 
    ksa[j] = [ a, ksa[i] = b ][0], c = (ksa[i << 5 ^ j >> 3] + ksa[j << 5 ^ i >> 3]) % 256, 
